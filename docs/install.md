...
```text
Foreman install check: status=ok version=1.5.39 entry=$cap registration=matching_registration config=present skill=matching_install agents=matching_install package_surface=coherent_surface companion_mcps=0 model_policy=coherent tool_policy=coherent run_hygiene=clean
Current package: codex-foreman@1.5.39
