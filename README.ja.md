...
> **CCC の位置づけ:** CCC は、Codex CLI で大きな開発作業を計画、調査、実装、レビューの流れに分け、`$cap` ひとつでロール別エージェントに任せるためのオーケストレーションツールです。進捗と担当ロールを見えやすくし、作業の性質に応じて低コストモデルと高性能モデルを使い分ける構成にして、トークンコストと作業失敗の可能性を下げることを目指します。
>
