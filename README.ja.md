...
> **CCC の位置づけ:** Codex CLI はすでに賢いツールです。CCC は、より大きな開発作業を captain-led の `$cap` 流れで整理したい Codex CLI ユーザー向けに、その賢さへ構造を加えます。計画、調査、実装、レビューをロール別エージェントに振り分け、進捗と担当を見えるまま保ち、作業ごとに適切なモデルを使うことでトークンコストを抑え、失敗リスクを下げます。
