...
CCC는 Codex CLI에서 큰 개발 작업을 계획, 조사, 구현, 검토 흐름으로 나누고, `$cap` 하나로 역할별 에이전트에게 맡기도록 돕는 오케스트레이션 도구입니다. 진행 상태와 담당 역할을 더 잘 보이게 하고, 작업 성격에 따라 저비용 모델과 고성능 모델을 나눠 쓰도록 구성해 토큰 비용과 작업 실패 가능성을 줄이는 것을 목표로 합니다.
