...
> 베타 안내: `0.0.4`는 아직 베타 릴리즈입니다. macOS가 우선 검증 경로입니다. Linux와 Windows 설치/실행 표면도 포함되어 있지만, 환경에 따라 아직 정상 동작하지 않을 수 있습니다.
...
`ccc status --text`와 quiet lifecycle line은 raw usage가 있으면 token gauge를 보여줍니다. Host custom subagent는 `ccc subagent-update`로 `context_tokens` 또는 `estimated_context_tokens`를 기록할 수 있습니다. raw token usage가 없고 context 추정치만 있는 경우 status는 `Context: <amount> estimated`, agent별 context breakdown, `token_usage_visibility.status=context_available`을 보여줍니다.
